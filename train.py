...
from model import ImageClassification, MyViT
...
for camera_folder in glob.glob(os.path.join(path_dataset, "cam_*")):
    sequences = [] 
    # images_path = os.path.join(path_dataset, camera_folder)
    for i in range(10):
    ...
    #train_sequences.append(sequences[num] for num in numbers_random[:8])
    val_sequences.append(sequences[numbers_random[8]])
    test_sequences.append(sequences[numbers_random[9]])

flattened_train = list(chain.from_iterable(list(chain.from_iterable(train_sequences))))
flattened_val = list(chain.from_iterable(val_sequences))
flattened_test = list(chain.from_iterable(test_sequences))
...
train_set = CustomDataset(path_images = flattened_train, classes = classes, data_frame=df)
val_set = CustomDataset(path_images = flattened_val, classes = classes,data_frame=df)
test_set = CustomDataset(path_images = flattened_test, classes = classes,data_frame=df)
